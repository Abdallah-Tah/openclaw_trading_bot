...
WALLET         = None  # Loaded dynamically from .trading_wallet.json
...
def load_wallet_address():
    """Load Solana wallet address from .trading_wallet.json"""
    global WALLET
    if WALLET_FILE.exists():
        try:
            from solders.keypair import Keypair
            data = json.loads(WALLET_FILE.read_text())
            kp = Keypair.from_bytes(bytes(data['secret_key']))
            WALLET = str(kp.pubkey())
            log(f"Loaded wallet: {WALLET}")
        except Exception as e:
            log(f"Error loading wallet: {e}")
            WALLET = "J6nK35ud8u6hzqDxuEtVWPsAMzv2v7H6stsxXW2rsnuH"  # fallback
    return WALLET
...
# Load wallet from .trading_wallet.json at startup
load_wallet_address()
