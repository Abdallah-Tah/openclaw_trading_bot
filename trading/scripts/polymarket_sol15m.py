...
LOG_F        = WORK_DIR / ".poly_sol15m.log"
LOCK_F       = WORK_DIR / ".poly_sol15m.lock"
...
MAKER_CANCEL_SEC = _int("SOL15M_MAKER_CANCEL_SEC", 30)
MAKER_OFFSET = _float("SOL15M_MAKER_OFFSET", 0.005)
MAKER_POLL_SEC = _int("SOL15M_MAKER_POLL_SEC", 10)
MAKER_MIN_PRICE = _float("SOL15M_MAKER_MIN_PRICE", 0.01)
...
SIGNAL_MAX_ENTRY_PRICE = _float("SOL15M_SIGNAL_MAX_ENTRY_PRICE", 0.85)  # skip if entry worse than this
SNIPE_DEFAULT   = _float("SOL15M_SNIPE_DEFAULT_SIZE", 3.00)  # RESTORED to $3 - user approved only BTC/ETH at $8
SNIPE_STRONG    = _float("SOL15M_SNIPE_STRONG_SIZE", 3.00)  # RESTORED to $3 - SOL not proven yet
SNIPE_STRONG_D  = _float("SOL15M_SNIPE_STRONG_DELTA", 0.10)  # percent
SNIPE_WINDOW    = _int("SOL15M_SNIPE_WINDOW_SEC", 30)
POLL_SEC        = _int("SOL15M_PRICE_POLL_SEC", 5)
SCAN_SEC        = _int("SOL15M_SCAN_INTERVAL", 10)
MAX_DAILY_LOSS  = _float("SOL15M_MAX_DAILY_LOSS", 15.00)
SERIES_ID       = "10192"
...
_state = {
    ...
    "arb_done":        False,
    "snipe_done":      False,
    "sol_prices":      [],   # list of (timestamp, price)
    "daily_pnl":       0.0,
    "daily_reset":     "",
    "trades":          [],
    "maker_order_id":   "",
...
def _track_dry_fill(direction, shares, price, notes="maker_dry"):
    _positions.append({
        "direction": direction,
        "shares": shares,
        "entry_price": price,
        ...
        "window_ts": _state.get("window_ts", 0),
        "slug": _state.get("prev_slug", ""),
        "resolved": False,
        "notes": notes,
    })


def _resolve_dry_positions(market):
    if not _positions:
        return
    yes = float(market.get("yes_price", 0) or 0)
    no = float(market.get("no_price", 0) or 0)
    winners = []
    if yes >= 0.99:
        winners.append("UP")
    if no >= 0.99:
    ...
    for pos in _positions:
        if pos.get("resolved"):
            continue
        if pos.get("window_ts") != _state.get("window_ts"):
            win = pos.get("direction") in winners
            payout = pos.get("shares", 0) * (1.0 if win else 0.0)
            ...
            log_trade(
                ...
                pnl,
                'resolved',
                int(time.time()) - pos.get("opened_ts", int(time.time())),
                notes=f"dry_resolved {pos.get('notes','')} outcome={outcome} slug={pos.get('slug','')}"
            )
            pos["resolved"] = True
...
def maker_place_order(side, shares, price, condition_id, token_id):
    ...
    if MAKER_DRY_RUN:
        log(f"[SOL-MAKER-DRY] {side} {shares} @{price:.4f} token={token_id}")
...
def check_maker_snipe(market, seconds_remaining):
    ...
    if oid and (time.time() - _state.get("maker_last_poll", 0) >= MAKER_POLL_SEC or seconds_remaining <= MAKER_CANCEL_SEC):
        ...
        log(f"[SOL-MAKER] status order_id={oid} -> {st.get('status')}")
    ...
    if not _state.get('window_open_sol'):
        return None
    delta_pct = (base_price - _state['window_open_sol']) / _state['window_open_sol'] * 100
    log(f"[SOL-MAKER] now={base_price} open={_state['window_open_sol']} delta={delta_pct:+.3f}% sec_rem={seconds_remaining}")
    ...
    recent_prices = [(t, p) for t, p in _state.get('sol_prices', []) if t >= cutoff_ts]
    if len(recent_prices) < SIGNAL_CONFIRM_COUNT:
        log(f"[SOL-MAKER] Confirm: only {len(recent_prices)}/{SIGNAL_CONFIRM_COUNT} samples, waiting")
        return None
    confirmations = 0
    for _, p in recent_prices[-SIGNAL_CONFIRM_COUNT:]:
        d = (p - _state['window_open_sol']) / _state['window_open_sol'] * 100
        if (direction == 'UP' and d > SNIPE_DELTA_MIN) or (direction == 'DOWN' and d < -SNIPE_DELTA_MIN):
            confirmations += 1
    if confirmations < SIGNAL_CONFIRM_COUNT:
        log(f"[SOL-MAKER] Confirm: {confirmations}/{SIGNAL_CONFIRM_COUNT}, skipping")
        return None
    recent_avg = sum(p for _, p in recent_prices[-SIGNAL_CONFIRM_COUNT:]) / len(recent_prices[-SIGNAL_CONFIRM_COUNT:])
    momentum = (recent_avg - _state['window_open_sol']) / _state['window_open_sol'] * 100
...
def check_gabagool(market):
    if not SOL15M_GABAGOOL_ENABLED:
        return None
...
def _acquire_lock():
    ...
    if LOCK_F.exists():
        try:
            existing_pid = int(LOCK_F.read_text().strip())
            if existing_pid and existing_pid != current_pid:
                os.kill(existing_pid, 0)
                log(f"[SOL-15M] Another instance already running (pid {existing_pid}), exiting")
                return False
        except (ValueError, ProcessLookupError, OSError):
            pass
    LOCK_F.write_text(str(current_pid))
...
def main():
    ...
    while True:
        ...
        if window_ts != _state["window_ts"]:
            prev_slug = _state.get("prev_slug", "")
            if prev_slug:
                prev_market = get_market(prev_slug)
                if prev_market:
                    _resolve_dry_positions(prev_market)
